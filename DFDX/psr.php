...
if (stripos($_SERVER['REQUEST_URI'],'news')===false) { 
    if (file_exists('image/psr.png'))
        echo '<img src="image/psr.png" alt="psr">';
    else psr();
}
...
$bylPoisk=false;
$action='action=psr.php';  //страница обработки кнопок в модуле news()
$nameBD='bd2';
$nameBD='nameTD='.$nameBD;
////////////////////////////////////////////////////поиск
if (isset($_POST['poisk'])) {
  $poisk->poiskStati('bd2',$_POST['strPoisk'],$idStati,'категория-psr') ;
  if ($idStati[0]>-1)
...
///////////////////////////////////////////////////
 if (!$bylPoisk) {
      ...
      if ($statiaPoId=='netKnopki' )  // Если не была нажата кнопка правой панели
        $redaktor->news1($nameBD,"Заголовок=h3","Статус редактора=-s12345","Шаблон=2","Отступ=1",$action,'Раздел=psr');
      if ($statiaPoId>-1 && $statiaPoId!='netKnopki') // Если была нажата кнопка правой панели
        $redaktor->news1("id=".$statiaPoId,$nameBD,"Заголовок=h3","Статус редактора=-s12345","Шаблон=2","Отступ=1",$action,'Раздел=psr');
  }
echo '</div>';
//////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////
echo '<div class="col-xl-2 col-lg-2 col-md-12 col-sm-12 col-12 prawy">';  // правое меню
 echo '<div class="poiskDiv">';
  poiskDfdx("psr.php"); // кнопка Поиск, ссылка на обработчик
 echo '</div>';
pravoePole("psr");   // категория статей, которые должны быть показаны в правом меню 
echo '</div>';
echo '</div>';
echo '</section>';
////////////////////////////////////////////////////////////////////////////////////////////////// 
...
$statistik->metkaStatistika("psr");   // метка для подсчёта загрузки страницы, пишется в БД
echo '<div class="futterDivDfdx">';
echo '<p class="footerMarginTop">Просмотров:'.$statistik->getMetkaStatistik("psr").'</p>'; // метка для подсчёта загрузки страницы, пишется в БД
