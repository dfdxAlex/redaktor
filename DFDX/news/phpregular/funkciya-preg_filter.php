...
$redaktor=new redaktor\modul();
$action='action=funkciya-preg_filter-.php';  //страница обработки кнопок в модуле news()
    ...
    {// echo 'ловим кнопку';
      ...
      if ($statiaPoId>-1 && $statiaPoId!='netKnopki') // Если была нажата кнопка правой панели
    ...
    } else // добавить if (false)   //маркер 
       {
        ...
        $redaktor->news1("id=4","nameTD=bd2","Заголовок=h3","Статус редактора=-s45","Статус редактора=-s12345","Шаблон=2","Отступ=1",$action);
       }
  }
  buttonTwitter("Функция : preg_filter(). http://dfdx.uxp.ru/news/phpregular/funkciya-preg_filter.php");
echo '</div>';
