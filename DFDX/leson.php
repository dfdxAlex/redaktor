...
//$_SESSION['redaktiruem']="leson.php";
...
{ 
if (file_exists('image/leson.png'))
 echo '<img src="image/leson.png" alt="leson">';
 else leson();
}
...
$redaktor=new redaktor\modul();
$action='action=leson.php';  //страница обработки кнопок в модуле news()
$nameBD='bd2';
$nameBD='nameTD='.$nameBD;
////////////////////////////////////////////////////поиск
if (isset($_POST['poisk']))
 {
  $poisk->poiskStati('bd2',$_POST['strPoisk'],$idStati,'категория-leson') ;
  if ($idStati[0]>-1)
...
///////////////////////////////////////////////////
 if (!$bylPoisk)
 {
      contentLeson(); // выводит содержание страниц с тестированием проектов
      $statiaPoId=$maty->hanterButton("false=netKnopki","rez=hant","nameStatic=panelPrawa","returnNameDynamic");
      if ($statiaPoId=='netKnopki' )  // Если не была нажата кнопка правой панели
        $redaktor->news1($nameBD,"Заголовок=h3","Статус редактора=-s12345","Шаблон=2","Отступ=1",$action,'Раздел=leson');
      if ($statiaPoId>-1 && $statiaPoId!='netKnopki') // Если была нажата кнопка правой панели
        $redaktor->news1("id=".$statiaPoId,$nameBD,"Заголовок=h3","Статус редактора=-s12345","Шаблон=2","Отступ=1",$action,'Раздел=leson');
  }
...
echo '<div class="col-xl-2 col-lg-2 col-md-12 col-sm-12 col-12 prawy">';  // правое меню
 ...
 echo '<div class="poiskDiv">';
  poiskDfdx("leson.php"); // кнопка Поиск, ссылка на обработчик
 echo '</div>';

pravoePole("leson");   // категория статей, которые должны быть показаны в правом меню 
echo '</div>';
...
$statistik->metkaStatistika("leson");   // метка для подсчёта загрузки страницы, пишется в БД
echo '<div class="futterDivDfdx">';
echo '<p class="footerMarginTop">Просмотров:'.$statistik->getMetkaStatistik("leson").'</p>'; // метка для подсчёта загрузки страницы, пишется в БД
