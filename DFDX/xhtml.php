...
//$_SESSION['redaktiruem']="xhtml.php";
...
if (!stripos('sss'.$_SERVER['REQUEST_URI'],'news')>0)
{ 
if (file_exists('image/xhtml.png'))
 echo '<img src="image/xhtml.png" alt="xhtml">';
 else xhtml();
}
...
$redaktor=new redaktor\modul();
$action='action=xhtml.php';  //страница обработки кнопок в модуле news()
$nameBD='bd2';
$nameBD='nameTD='.$nameBD;
////////////////////////////////////////////////////поиск
if (isset($_POST['poisk']))
 {
  $poisk->poiskStati('bd2',$_POST['strPoisk'],$idStati,'категория-xhtml') ;
  if ($idStati[0]>-1)
 ...
 if (!$bylPoisk)
 {
   ...
   /// {// echo 'ловим кнопку';
      ...
      if ($statiaPoId=='netKnopki' )  // Если не была нажата кнопка правой панели
        $redaktor->news1($nameBD,"Заголовок=h3","Статус редактора=-s12345","Шаблон=2","Отступ=1",$action,'Раздел=xhtml');
      if ($statiaPoId>-1 && $statiaPoId!='netKnopki') // Если была нажата кнопка правой панели
        $redaktor->news1("id=".$statiaPoId,$nameBD,"Заголовок=h3","Статус редактора=-s12345","Шаблон=2","Отступ=1",$action,'Раздел=xhtml');
   // } else // добавить if (false)   //маркер 
    //   {
     //   $id=$poisk->maxIdLubojTablicy($nameBD);$id--;
     //   $redaktor->news1("id=".$id,$nameBD,"Заголовок=h3","Статус редактора=-s45","Статус редактора=-s12345","Шаблон=2","Отступ=1",$action,'Раздел=xhtml');
     //  }
  }
...
echo '<div class="col-xl-2 col-lg-2 col-md-12 col-sm-12 col-12 prawy">';  // правое меню
 ...
 echo '<div class="poiskDiv">';
  poiskDfdx("xhtml.php"); // кнопка Поиск, ссылка на обработчик
 echo '</div>';

pravoePole("xhtml");   // категория статей, которые должны быть показаны в правом меню 
echo '</div>';
...
$statistik->metkaStatistika("xhtml");   // метка для подсчёта загрузки страницы, пишется в БД
echo '<div class="futterDivDfdx">';
echo '<p class="footerMarginTop">Просмотров:'.$statistik->getMetkaStatistik("xhtml").'</p>'; // метка для подсчёта загрузки страницы, пишется в БД
