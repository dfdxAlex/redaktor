...
interface InterfaceButton extends InterfaceWorkToBd
{
   ...
   // Добавлено: textL и textLH - текстовое поле с использованием тега LABEL
   // первый параметр - это имя текстового поля
   // второй параметр - это отображение содержимого в теге LABEL
   // третий параметр - это содержимое атрибута placeholder в текстовом поле
   // class="$nameBlock+name+номер кнопки", по умолчанию placeholder будет пустая строка
   ...
   // Добавлен id, который равен атрибуту name
   ...
   //buttonUrl ссылка, всё что в кнопке submit3, только без имени 
   ...
   // добавлен ID, который равен имени класса абзаца

   // добавлен div
   // первый параметр - это содержимое дива
   // второй параметр - это класс
   // третий параметр - это ID
   ...
   // добавлен тег color, второй параметр - это имя

   // добавлены checkbox и radio, второй параметр имя кнопки, третий - надпись перед кнопкой, четвертый ID
   // если к параметру name добавить -checked , то будет отмеченная галочка

   // добавлены списки ol, ul, dl
   // первый параметр - это класс, для ol и ul и dl он не изменется, для li добавляется нумерация, если '' то класса не будет
   ...
   // второй параметр - это id, для select он не изменется, для option добавляется нумерация, если '' то id не будет
   // третий параметр - это имя атрибута NAME
   // четвертый параметр - это строка, содержащая дополнительную инфу. Если в ней будет слово multiple, то оно добавится как 
